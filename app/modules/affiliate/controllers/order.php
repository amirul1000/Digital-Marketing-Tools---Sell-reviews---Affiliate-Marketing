...
class order extends MX_Controller {
	...
	public function __construct(){
		...
		if (get_role("affiliate") || get_role("supporter")) {
			$this->columns = array(
				"order_id"                  => lang("order_id"),
				"date"                      => lang("Date"),
				"time"                      => lang("Time"),
				...
				// "charge"                    => lang("Charge"),
				"price"                     => lang("Price"),
				"start_counter"             => lang("Start_counter"),
				"remains"                   => lang("Remains"),
				"type"                      => lang("Type"),
	...
	public function index($order_status = ""){
		...
		$this->template->build('order/logs/logs', $data);
	...
	public function log_update($ids = ""){
		...
		$this->load->view('order/logs/update', $data);
	}
	
	public function log_view($ids = ""){
		$order    = $this->model->get("*", $this->tb_order, "ids = '{$ids}'");
		$data = array(
			"module"   		=> get_class($this),
			"order" 	    => $order,
	...
	public function change_status($status = "", $ids = "" , $is_redirect = true){
		if (!get_role('user')) _validation('error', "Permission Denied!");
		if (!is_string($ids)) {
			redirect(cn('client/order/log'));
		}
		$data = [];
		...
		if ($is_redirect) {
			...
			if (!$redirect_url) {
				$redirect_url = cn('client/order/log');
			}
	...
	public function ajax_order_by($status = ""){
		if (!empty($status) && $status !="" ) {
			...
			$this->load->view("order/logs/ajax_search", $data);
		}
	}
