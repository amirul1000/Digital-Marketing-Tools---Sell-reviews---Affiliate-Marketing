...
class blocks extends MX_Controller {
	...
	public function sidebar(){
		...
		$data = array(
		    ...
		    'totalUsers'    => count($this->model->fetch('*', USERS, 'status = 1')),

		    'openTickets'   => count($this->model->fetch('*', TICKETS, 'status = "new"')),

		    'totalPayments'   => count($this->model->fetch('*', TRANSACTION_LOGS, '')),

		    'totalOrders'   => count($this->model->fetch('*', ORDER, '')),
		...
		if($user_current_info['role']=='user'){

			$this->load->view('sidebar_client', $data);
		
		}		
		else if($user_current_info['role']=='affiliate'){

			$this->load->view('sidebar_affiliate', $data);
		
		} 
		else {//if($user_current_info['role']=='admin')
	...
	public function header_vertical(){
		...
		if($user_current_info['role']=='user'){
			...
			$this->load->view('header_vertical_client', $data);
		
		}
		else if($user_current_info['role']=='affiliate'){

			$this->load->view('header_vertical_affiliate', $data);
	...
	public function user_header_new(){

		$categories = $this->category_model->get_active_category_lists(true);
	...
	public function user_header_top_new($link = ''){
		...
		$this->load->view("user/header_top_new", $data);
