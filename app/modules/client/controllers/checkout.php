...
// 		}
		...
		if (get_theme() == 'review') {
    		...
    		$this->template->build("checkout/index_new", $data);
		} else {
    		//$this->template->set_layout('user');
    		$this->template->build("checkout/index", $data);
		}
	}
	...
	public function process(){
		...
		if($item->is_free){
			...
			if(isset($save_order->status) && $save_order->status == 'success') {
				$redirect_url = cn("checkout/checkout/success/".$save_order->transaction_id);
	...
	public function success($id = ""){
		...
		if (!empty($transaction)) {
			...
			$this->template->set_layout('user');
			$this->template->build("checkout/payment_successfully", $data);
		}else{
			redirect(cn());
		}
	}
	...
	public function unsuccess(){
		...
		$this->template->set_layout('user');
		$this->template->build("checkout/payment_unsuccessfully", $data);
	}
