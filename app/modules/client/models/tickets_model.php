...
class tickets_model extends MY_Model {
	...
	function get_tickets($total_rows = false, $status = "", $limit = "", $start = ""){
		$this->db->select('tk.*, o.ids as order_ids, o.created as order_created');
		$this->db->from($this->tb_tickets." tk");
		$this->db->join($this->tb_orders." o", "o.id = tk.order_id", 'left');
	...
	function get_ticket_by_ids($ids) {
		$this->db->select('tk.*, o.ids as order_ids, o.created as order_created');
		$this->db->from($this->tb_tickets." tk");
		$this->db->join($this->tb_orders." o", "o.id = tk.order_id", 'left');
		$this->db->where("tk.ids", $ids);
		$this->db->order_by('tk.changed', 'DESC');
		$query = $this->db->get();
		if($query->row()){
	...
	function get_ticket_detail($id){
		...
		$this->db->select('tk.*, u.email as user_email, u.first_name, u.last_name,u.role, o.ids as order_ids, o.created as order_created');
		$this->db->from($this->tb_tickets." tk");
	...
	public function get_count_tickets_by_search($search = []){
		...
		switch ($search['type']) {
			...
			case 2:
				# User Email
				$where_like = "`tk`.`email` LIKE '%".$k."%' ESCAPE '!'";
				break;

			case 3:
				# Department
				$where_like = "`tk`.`department` LIKE '%".$k."%' ESCAPE '!'";
				break;
		}
