      ...
      <table class="table table-hover table-bordered table-vcenter card-table">
        ...
        <tbody>
          ...
          <tr class="tr_<?php echo strip_tags($row->ids); ?>">
            ...
            <td>
              <?php echo !empty($row->total_spent)
                  ? $currency_symbol .
                      ' ' .
                      currency_format(
                          $row->total_spent,
                          get_option('currency_decimal', 2),
                          $decimalpoint,
                          $separator
                      )
                  : 0; ?>
            </td>
            <td class="text-muted"><?php echo strip_tags(
                $row->history_ip
            ); ?></td>
            <td class="text-muted"><?php html_entity_decode(
                $row->description,
                ENT_QUOTES
            ); ?></td>
            <td><?php echo convert_timezone($row->changed, 'user'); ?></td>
            <td class="text-muted"><?php echo convert_timezone(
                $row->created,
                'user'
            ); ?></td>
            <td class="text-center">
              <div class="btn-group">
                ...
                ); ?>" class="btn btn-icon btn-azure ajaxModal" data-toggle="tooltip" data-placement="bottom" title="<?php echo lang(
    'Add_Note'
); ?>"><i class="fe fe-edit"></i></a>
                <a href="<?php echo cn(
                    $module . '/mail/' . $row->ids
                ); ?>" class="btn btn-icon btn-info ajaxModal" data-toggle="tooltip" data-placement="bottom" title="<?php echo lang(
    'send_mail'
); ?>"><i class="fe fe-mail"></i></a>
                <a href="<?php echo cn(
                    "$module/ajax_delete_item/" . $row->ids
                ); ?>" class="btn btn-icon btn-danger ajaxDeleteItem" data-toggle="tooltip" data-placement="bottom" title="<?php echo lang(
    'Delete'
