...
<div class="page-header">
  <h3>
    <?php echo ($category) ? 'Edit Network: ' .$category->name : 'Add New'; ?>
  </h3>
</div>
  ...
  <div class="col-md-6">
    <form class="form actionForm" action="<?php echo strip_tags($url); ?>" method="POST" data-redirect="<?php echo get_current_url(); ?>">
      <div class="card">
        ...
        <div class="card-footer m-t-20 d-flex justify-content-between">
          <button type="submit" class="btn btn-primary btn-min-width mr-1 mb-1"><?php echo lang('Save'); ?></button>
          <a href="<?= cn('social_network') ?>" class="btn btn-secondary btn-min-width mr-1 mb-1"><?php echo lang('Cancel'); ?></a>
        </div>
      </div>
      </form>
  </div>
